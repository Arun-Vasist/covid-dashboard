...
tooltip_text = {
    'vaccinations_per_million': 'SUM(Vaccinations) / SUM(Population) * 1M',
...
def generate_kpi_card_body(click_data, metric, state_metrics_df):
    if not click_data:
        ...
        vs_national_avg = html.H3(
            f"{print_delta(now=result, prev=state_metrics_df.loc['India', metric])} vs National Avg",
                                  style={'color': color, 'font-size': 34})
    ...
    card_body = dbc.CardBody([
                    ...
                    html.H1(print_formatted(result, metric), style={'font-weight': 'bold', 'font-size': 45}),
                    vs_national_avg,
                    # rank,
...
def create_app(date_wise_metrics, state_metrics_df, india_df, india_geojson):
    ...
    card_style = {'margin-bottom': '20px', 'padding-bottom': '0px'}
    vaccinations_per_million_card = dbc.Card(id='vaccinations_per_million_card', style=card_style)
    ...
    @app.callback(
        [
            Output("vaccinations_per_million_card", "children"),
    ...
    def render_page_content(click_data, _):
        ...
        if button_id == 'choropleth':
            vaccinations_kpi_card = generate_kpi_card_body(click_data, "vaccinations_per_million", state_metrics_df)
            ...
            vaccinations_kpi_card = generate_kpi_card_body(None, "vaccinations_per_million", state_metrics_df)
